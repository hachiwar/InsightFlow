...
| 能力 | 状态 | 说明 |
|---|---|---|
| MindAgent 统一 `/chat` 入口 | 已实现 | Java Spring Boot 接口 |
| 客服意图识别 | 已实现 | LLM、字符相似度和模式规则融合 |
| `DATA_QUERY` 意图 | 已接通 | 已加入模板、模式词和路由映射 |
| General、Technical、Billing Agent | 已实现 | MindAgent 原有能力 |
| DataAgent | 已接通 | 通过 HTTP 调用 DataAgent |
| DataAgent 健康检查 | 已接通 | `GET /health` |
| DataAgent 查询接口 | 已接通 | `POST /query` |
| 公开 API 鉴权 | 已接通 | 容器部署强制使用 `X-API-Key` |
| DataAgent 内部鉴权 | 已接通 | MindAgent 使用独立的 `X-Internal-API-Key` |
| 字段级 Schema 检索 | 已实现 | DataAgent 核心代码 |
| 关键词与向量混合召回 | 已实现 | 当前使用本地哈希向量作为可运行降级实现 |
| RRF 融合 | 已实现 | DataAgent 检索链路 |
| 远程 Rerank | 部分实现 | 需要有效服务配置；否则使用降级排序 |
| SchemaGraph | 已实现 | DataAgent 检索结果构建 |
...
- DataAgent 和 MindAgent Docker 镜像构建通过；
- 根级 `compose.yaml` 解析通过；
- 范例站 3 个复杂查询场景和 SQL 只读拦截测试通过；
- Vite 生产构建通过；
- `GET /health` 返回 `{"status":"ok"}`；
- 公开接口未授权返回 `401`，DataAgent 非法请求返回 `400`；
- Caddy → MindAgent → DataAgent → SQLite 端到端查询通过，返回 `agent_type=data`；
- Redis、DataAgent 和 MindAgent 未向主机公开端口。

## 后续顺序

1. 增加 DataAgent 与 DataAgent 的独立 HTTP 集成测试。
2. 增加数据意图冲突用例和路由评测。
3. 定义稳定的数据查询响应结构。
