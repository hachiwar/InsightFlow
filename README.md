...
InsightFlow 面向企业私域数据，将 MindAgent 的统一 API、会话管理和 Agent 编排能力与 DataAgent 的结构化数据查询能力组合起来，根据用户意图在知识库问答、业务服务和数据库分析之间选择处理链路。
...
仓库包含可发布到 GitHub Pages 的完整项目展示站。页面通过交互式路由演示说明 MindAgent 的统一 `/chat` 入口、会话记忆、知识库 RAG、意图识别、General/Technical/Billing/Data 四类 Agent、失败降级与监控评测，并展示 MindAgent、DataAgent、Redis、Caddy 的部署边界。数据实验室会在浏览器内真实运行 SQLite，逐步展示关键词识别、Schema 召回、查询规划、SQL 生成、只读检查、执行结果和结果解释。
...
GitHub Pages 只能执行前端 JavaScript 和 WebAssembly。页面对完整后端流程进行如实的架构展示，但不会在静态站点内启动 MindAgent Java、DataAgent Python、Redis、Caddy 或 Docker。完整后端必须按服务器上线指南部署。
...
```text
InsightFlow/
├── dataagent/             # Python Text2SQL 核心链路
├── mindagent/            # Java 业务 Agent 与统一 API
...
只有 Caddy 向主机映射 `80` 和 `443` 端口。Redis、DataAgent 和 MindAgent 仅通过 Compose 内部网络通信。

## 本地运行

### 启动 DataAgent

在 `dataagent/code/dataagent_agent/dataagent_agent` 目录运行：

```powershell
$env:PYTHONIOENCODING = "utf-8"
$env:DATAAGENT_ALLOW_MOCK = "true" # 仅用于文档演示查询
...
在 `mindagent/MindAgentJava/MindAgentJava` 目录配置模型和 Redis，然后运行 Spring Boot 应用。MindAgent 默认通过 `http://localhost:8090` 调用 DataAgent。

可用环境变量：

| 变量 | 默认值 | 说明 |
|---|---:|---|
| `DATAAGENT_BASE_URL` | `http://localhost:8090` | DataAgent 服务地址 |
| `DATAAGENT_TIMEOUT_MS` | `30000` | 数据查询超时时间，单位为 ms |
| `DATAAGENT_API_KEY` | 空 | MindAgent 访问 DataAgent 的内部密钥 |
| `MINDAGENT_AUTH_ENABLED` | `false` | 是否开启公开 API 鉴权 |
| `MINDAGENT_API_KEY` | 空 | 公开 API 的 `X-API-Key` 密钥 |
...
- DataAgent 当前只注册 SQLite 演示执行器。
- SQLite 以只读模式执行，单次查询设置超时并最多返回 100 行。
- 容器部署默认禁用 DataAgent Mock，未配置有效模型时数据查询失败关闭。
- 公开 API 使用 `X-API-Key`，MindAgent 调用 DataAgent 使用独立的 `X-Internal-API-Key`。
